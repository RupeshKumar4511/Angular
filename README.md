...
When we declare a property inside a class in Typescript then we do not need to use "let or const or var" with it and if we do not specify the datatype then also it works. 
<br>
But when we declare variable inside method then we need to use "let or const or var " with it. 
<br>
```bash 
